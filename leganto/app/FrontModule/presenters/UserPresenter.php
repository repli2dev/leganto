...
class UserPresenter extends BasePresenter {
	...
	public function renderIcon($id) {
		...
		$data = Factory::opinion()->getSelector()->findAllByUser(
				$user
			)->orderBy("updated", "DESC")->applyLimit(3)->fetchAll();
		$books = array();
		foreach ($data as $row) {
			$books[] = Factory::book()->getSelector()->find($row->id_book_title);
		}
		...
		if (count($books) > 2) {
			...
			$author = Factory::author()->getSelector()->findAllByBook($books[1])->fetchAll();
			$authorInsert = $author[0]->full_name;
			if (count($author) != 1) {
				$authorInsert = $authorInsert . "\xE2\x80\xA6";
				$authorInsert = ExtraString::hardTruncate($authorInsert, 18, "");
			...
			$author = Factory::author()->getSelector()->findAllByBook($books[2])->fetchAll();
			$authorInsert = $author[0]->full_name;
			if (count($author) != 1) {
				$authorInsert = $authorInsert . "\xE2\x80\xA6";
