...
extends
IOException{}}if(!interface_exists('IDebugPanel',FALSE)){interface
...
DibiResult($res,$this->config);}else{$res=$this->driver->getAffectedRows();}if(isset($ticket)){$this->profiler->after($ticket,$res);}return$res;}public
function
...
function
__construct($driver,$config){$this->driver=$driver;if(!empty($config[dibi::RESULT_DETECT_TYPES])){$this->detectTypes();}if(!empty($config[dibi::RESULT_DATE_TIME])){$this->dateFormat=is_string($config[dibi::RESULT_DATE_TIME])?$config[dibi::RESULT_DATE_TIME]:'';}}final
public
function
...
public$maxLength=1000;private$file;public$useFirebug;private$filter=self::ALL;public
static$tickets=array();public
static$fireTable=array(array('Time','SQL Statement','Rows','Connection'));public
function
__construct(){if(class_exists('Debug',FALSE)&&is_callable('Debug::addPanel')){Debug::addPanel($this);}$this->useFirebug=isset($_SERVER['HTTP_USER_AGENT'])&&strpos($_SERVER['HTTP_USER_AGENT'],'FirePHP/');}public
function
...
REVISION='550be3b released on 2010-04-06';const
RESULT_DETECT_TYPES='resultDetectTypes';const
RESULT_DATE_TIME='resultDateTime';const
