...
FOR EACH ROW
	INSERT INTO `feed_event` (type, id_user, inserted, content) VALUES (
		...
		NEW.`id_user`,
		NOW(),
		(SELECT CONCAT_WS('#$#',`id_book_title`,`book_title`,`content`,`rating`) FROM `view_opinion` WHERE `id_opinion` = NEW.`id_opinion`)
	);
...
FOR EACH ROW
	INSERT INTO `feed_event` (type, id_user, inserted, content) VALUES (
		...
		NEW.`id_user`,
		NOW(),
		(SELECT CONCAT_WS('#$#',`id_book_title`,`book_title`,`content`,OLD.`content`,`rating`,OLD.`rating`) FROM `view_opinion` WHERE `id_opinion` = NEW.`id_opinion`)
	);
...
FOR EACH ROW
	INSERT INTO `feed_event` (type, id_user, inserted, content) VALUES (
		...
		NOW(),
		(SELECT CONCAT_WS('#$#',`type`,`name`,`id_book_title`,`title`,`subtitle`) FROM `view_shelf_book` WHERE `id_in_shelf` = NEW.`id_in_shelf`) 
	);
...
FOR EACH ROW
	INSERT INTO `feed_event` (type, id_user, inserted, content) VALUES (
		...
		NOW(),
		(SELECT CONCAT_WS('#$#',`type`,`name`,`id_book_title`,`title`,`subtitle`) FROM `view_shelf_book` WHERE `id_in_shelf` = OLD.`id_in_shelf`) 
	);
...
FOR EACH ROW
	INSERT INTO `feed_event` (type, id_user, inserted, content) VALUES (
		...
		NEW.`id_user`,
		NOW(),
		(SELECT CONCAT_WS('#$#',`id_user`,`nick`) FROM `user` WHERE `id_user` = NEW.`id_user_followed`) 
	);
